...
- Add `package:web3libdart/browser.dart`, a library for using this package in
...
- Add the `package:web3libdart/contracts/erc20.dart` library for interacting with an
...
- Fix `hexToDartInt` not actually parsing hex ([#81](https://github.com/simolus3/web3libdart/issues/81))
...
- Fixed a bug when decoding negative integers ([#73](https://github.com/simolus3/web3libdart/issues/73))
...
| Low-level cryptographic operations like signing, hashing and converting hex <-> byte array <-> integer  | Not available in the core library. Import `package:web3libdart/crypto.dart` instead |

If you run into problems after updating, please [create an issue](https://github.com/simolus3/web3libdart/issues/new).
